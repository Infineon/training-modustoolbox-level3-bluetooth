...
static wiced_bt_gatt_status_t app_bt_connect_event_handler(wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    if (NULL != p_conn_status)
    {
        if (p_conn_status->connected)
        {
			...
			/* Save the remote bd_addr into bondinfo because, at this point, we know that is good data */
            memcpy( bondinfo.link_keys.bd_addr, p_conn_status->bd_addr, sizeof( wiced_bt_device_address_t ) );
        }
        else
        {
