...
int main(void)
{
	...
	#ifdef ENABLE_OTA
		/* set default values for ota context */
		ota_initialize_default_values();
...
static wiced_bt_gatt_status_t app_bt_server_event_handler(wiced_bt_gatt_event_data_t *p_data)
{
    ...
    switch (p_att_req->opcode)
    {
		...
		case GATT_HANDLE_VALUE_CONF: /* Value confirmation */
			{
				#ifdef ENABLE_OTA
					cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "  %s() GATTS_REQ_TYPE_CONF\r\n",
							__func__);
...
static wiced_bt_gatt_status_t app_bt_write_handler(wiced_bt_gatt_event_data_t *p_data)
{
	...
	wiced_bt_gatt_status_t status = WICED_BT_GATT_SUCCESS;
		...
		switch (p_write_req->handle)
		{
			case HDLD_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_CLIENT_CHAR_CONFIG:
				ota_app.bt_ota_config_descriptor = p_write_req->p_val[0];
				status = WICED_BT_GATT_SUCCESS;
				return status;
				break;
			...
			case HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_VALUE:
				switch(p_write_req->p_val[0])
				{
					case CY_OTA_UPGRADE_COMMAND_PREPARE_DOWNLOAD:
						...
						result = cy_ota_ble_download_prepare(ota_app.ota_context, connection_id, ota_app.bt_ota_config_descriptor);
						if (result != CY_RSLT_SUCCESS)
						{
							status= WICED_BT_GATT_ERROR;
							break;
					...
					case CY_OTA_UPGRADE_COMMAND_DOWNLOAD:
						result = cy_ota_ble_download(ota_app.ota_context, p_data, connection_id, ota_app.bt_ota_config_descriptor);
						if (result != CY_RSLT_SUCCESS)
						{
							status = WICED_BT_GATT_ERROR;
							break;
					...
					case CY_OTA_UPGRADE_COMMAND_VERIFY:
						result = cy_ota_ble_download_verify(ota_app.ota_context, p_data, connection_id);
						if (result != CY_RSLT_SUCCESS)
						{
							status = WICED_BT_GATT_ERROR;
							break;
	...
	/* Normal GATT write handling - we only get here if this wasn't for an OTA service characteristic */
    status= WICED_BT_GATT_ERROR;
