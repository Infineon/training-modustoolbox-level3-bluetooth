...
uint16_t ledHandle = 0x09u; // This is found in the peripheral's Bluetooth GATT configuration in cycfg_gatt_db.h
uint16_t cccdHandle = 0x0Cu;
uint16_t buttonCountHandle = 0x0Bu;
        ...
        if(pdPASS == xQueueReceive( xUARTQueue, &(readbyte), portMAX_DELAY))
        {
            switch (readbyte)
			{
				...
				case 'n':			// Turn notifications on
					{
						...
						writeAttribute( connection_id, cccdHandle, 0, GATT_AUTH_REQ_NO_MITM, sizeof(uint16_t), writeData);
					}
					break;
				...
				case 'r':			// Read LED value
					wiced_bt_gatt_client_send_read_handle(connection_id, ledHandle, 0, &ledValue, sizeof(ledValue), GATT_AUTH_REQ_NONE);
					break;
