...
/* Function to allow the stack to send BTSpy messages */
#ifdef ENABLE_BT_SPY_LOG
    ...
    switch( event )
    {
		case BTM_ENABLED_EVT:								// Bluetooth Controller and Host Stack Enabled
			if( WICED_BT_SUCCESS == p_event_data->enabled.status )
			{
                ...
                /* Register callback to print out HCI messages from the stack*/
                #ifdef ENABLE_BT_SPY_LOG
...
*
* This Function sends the BTSpy messages. It is called by the stack
*
********************************************************************************/
#ifdef ENABLE_BT_SPY_LOG
void hci_trace_cback(wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data)
