    ...
    switch( event )
    {
		...
		case BTM_ENCRYPTION_STATUS_EVT: 						// Encryption Status Event
	        ...
	        if( bond_mode == BONDED)
	    	{
				...
				for(count = 0; count < bondinfo.num_bonded; count++)
				{
					if(memcmp( bondinfo.link_keys[count].bd_addr, p_event_data->encryption_status.bd_addr, sizeof(wiced_bt_device_address_t))==0) /* Matching address found */
					{
						current_slot = count;
		...
		case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT: 			// Save link keys
			...
			current_slot = bondinfo.next_slot;
			/* Check to see if we already have data for a device with this BDA in an existing slot */
			for(count = 0; count < bondinfo.num_bonded; count++)
	        {
	        	if(memcmp( bondinfo.link_keys[count].bd_addr, p_event_data->paired_device_link_keys_update.bd_addr, sizeof(wiced_bt_device_address_t))==0) /* Matching address found */
	        	{
			...
			wiced_bt_dev_add_device_to_address_resolution_db ( &(bondinfo.link_keys[current_slot]) );
