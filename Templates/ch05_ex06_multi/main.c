...
#define EEPROM_NUM_BONDED		((void *)&(bondinfo.num_bonded) - (void *)&bondinfo)
#define EEPROM_NEXT_SLOT		((void* )&(bondinfo.next_slot) - (void *)&bondinfo)
...
/* Structure to store info that goes into EEPROM - it holds the CCCD values, remote keys, local keys,
 * number of bonded devices, and next slot to store bonding information in */
struct bondinfo
{
...
uint8_t current_slot = DEVICE_NOT_FOUND; 	/* Slot in bondinfo that has the information for the currently bonded device */
...
int main(void)
{
	...
	for(int i=0; i<BOND_MAX; i++)
	{
		...
		print_array(&bondinfo.link_keys[i].bd_addr, sizeof(bondinfo.link_keys[i].bd_addr));
		printf("CCCD: ");
		print_array(&bondinfo.cccd[i], sizeof(bondinfo.cccd[i]));
		printf("Link Keys: ");
		print_array(&bondinfo.link_keys[i], sizeof(bondinfo.link_keys[i]));
	}
	printf("********************\n");
    ...
    switch( event )
    {
		case BTM_ENABLED_EVT:								// Bluetooth Controller and Host Stack Enabled
			if( WICED_BT_SUCCESS == p_event_data->enabled.status )
			{
				...
				/* If bonding info was previously stored, copy in the keys and add them to the address resolution database */
				if(bondinfo.num_bonded > 0)
				{
					for(count = 0; count < bondinfo.num_bonded; count++)
					{
						wiced_bt_dev_add_device_to_address_resolution_db ( &bondinfo.link_keys[count]);
						printf("Added bonded BDA into address resolution database: ");
		...
		case BTM_PAIRING_COMPLETE_EVT: 							// Pairing Complete event
	        ...
	        {
				bond_mode = BONDED; // Exit bonding mode
				result = WICED_BT_SUCCESS;
	        }
			break;
		...
		case BTM_ENCRYPTION_STATUS_EVT: 						// Encryption Status Event
	        ...
	        if( bond_mode == BONDED)
	    	{
	        	...
	        	app_psoc_button_count_client_char_config[0] = bondinfo.cccd[current_slot][0];
	        	app_psoc_button_count_client_char_config[1] = bondinfo.cccd[current_slot][1];
		...
		case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT: 			// Save link keys
			...
			memcpy(&bondinfo.link_keys[current_slot], &(p_event_data->paired_device_link_keys_update), sizeof( wiced_bt_device_link_keys_t));
			eepromReturnValue = Cy_Em_EEPROM_Write(EEPROM_LINK_KEYS + (current_slot * sizeof(wiced_bt_device_link_keys_t)), &bondinfo.link_keys[current_slot], sizeof( wiced_bt_device_link_keys_t), &Em_EEPROM_context);
			if(CY_EM_EEPROM_SUCCESS == eepromReturnValue)
			{
				printf( "Keys saved to EEPROM for BDA ");
				print_bd_address(bondinfo.link_keys[current_slot].bd_addr);
			...
			{
				bondinfo.num_bonded++;
				bondinfo.next_slot = (bondinfo.next_slot + 1) % BOND_MAX; /* Increment but wrap around when we reach BOND_MAX */

				eepromReturnValue = Cy_Em_EEPROM_Write(EEPROM_NUM_BONDED, &(bondinfo.num_bonded), sizeof(bondinfo.num_bonded), &Em_EEPROM_context);
				if(CY_EM_EEPROM_SUCCESS != eepromReturnValue)
				{
					printf("------->EEPROM Write Error: %d\n", eepromReturnValue);
				}

				eepromReturnValue = Cy_Em_EEPROM_Write(EEPROM_NEXT_SLOT, &(bondinfo.next_slot), sizeof(bondinfo.next_slot), &Em_EEPROM_context);
				if(CY_EM_EEPROM_SUCCESS != eepromReturnValue)
				{
					printf("------->EEPROM Write Error: %d\n", eepromReturnValue);
				}
		...
		case BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT: 			// Retrieve saved link keys
			...
			current_slot = DEVICE_NOT_FOUND; /* Assume device is not yet bonded */

			for(count = 0; count < bondinfo.num_bonded; count++)
	        {
	        	if(memcmp( bondinfo.link_keys[count].bd_addr, p_event_data->paired_device_link_keys_request.bd_addr, sizeof(wiced_bt_device_address_t))==0) /* Matching address found */
	        	{
	        		current_slot = count;
					memcpy(&(p_event_data->paired_device_link_keys_request), &(bondinfo.link_keys[current_slot]), sizeof(wiced_bt_device_link_keys_t));
	        ...
	        if (current_slot == DEVICE_NOT_FOUND) /* Device was not found so we will enter the data in the next slot */
	        {
	        	printf("Link keys not found\n");
	        	current_slot = bondinfo.next_slot; /* This device's info will go in the next slot */
        		new_device = WICED_TRUE;

        		if(bondinfo.num_bonded == BOND_MAX) /* Need to remove oldest device */
        		{
        			/* Remove oldest device from the bonded device list */
        			wiced_bt_dev_delete_bonded_device(bondinfo.link_keys[bondinfo.next_slot].bd_addr);
        			printf( "Removed host from bonded device list: ");
        			print_bd_address(bondinfo.link_keys[bondinfo.next_slot].bd_addr);

        			/* Remove oldest device from address resolution database */
        			wiced_bt_dev_remove_device_from_address_resolution_db (&(bondinfo.link_keys[bondinfo.next_slot]));

        			/* Reduce number of bonded devices by one - this will be incremented again when keys are stored for the new device */
        			bondinfo.num_bonded--;
        		}
	        }

	        if(bond_mode == BONDED) /* Bonded mode, do not generate new keys */
	        {
        		result = WICED_BT_SUCCESS;
	        }
	        else /* Bonding mode, need to generate new keys */
	        {
	        	result = WICED_BT_ERROR;
	        }
...
static wiced_bt_gatt_status_t app_bt_connect_event_handler(wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    if (NULL != p_conn_status)
    {
        if (p_conn_status->connected)
        {
			...
			/* Save the remote bd_addr into bondinfo so it is available for numeric comparison response */
            memcpy( bondinfo.link_keys[current_slot].bd_addr, p_conn_status->bd_addr, sizeof( wiced_bt_device_address_t ) );
        }
        else
        {
             ...
             if (app_gatt_db_ext_attr_tbl[i].max_len >= p_write_req->val_len)
             {
                 ...
                 switch ( p_write_req->handle )
                 {
 					...
 					case HDLD_PSOC_BUTTON_COUNT_CLIENT_CHAR_CONFIG:
                    	...
                    	bondinfo.cccd[current_slot][0] = p_write_req->p_val[0];
 						bondinfo.cccd[current_slot][1] = p_write_req->p_val[1];
 						eepromReturnValue = Cy_Em_EEPROM_Write(EEPROM_CCCD + (current_slot * sizeof(bondinfo.cccd[current_slot])), &(bondinfo.cccd[current_slot]), sizeof(bondinfo.cccd[current_slot]), &Em_EEPROM_context);
 						if(CY_EM_EEPROM_SUCCESS != eepromReturnValue)
 						{
 							printf("------->EEPROM Write Error: %d\n", eepromReturnValue);
 						}
