...
void app_multi_advertise( void )
{
	...
	/* Set up a UID packet with ranging_data = 0xF0, and the uid_namespace and uid_instance values declared above */
    wiced_bt_eddystone_set_data_for_uid( UID_RANGE, uid_namespace, uid_instance, uid_packet, &packet_len );
    wiced_set_multi_advertisement_data( uid_packet, packet_len, ADV_INST_UID );
    wiced_set_multi_advertisement_params( ADV_INST_UID, &adv_parameters );
    wiced_start_multi_advertisements( MULTI_ADVERT_START, ADV_INST_UID );
    ...
    wiced_set_multi_advertisement_data( tlm_packet, packet_len, ADV_INST_TLM );
    wiced_set_multi_advertisement_params( ADV_INST_TLM, &adv_parameters );
    wiced_start_multi_advertisements( MULTI_ADVERT_START, ADV_INST_TLM );
}
...
void timer_cback( uint32_t data )
{
	...
	/* Reuse two lines of code from app_multi_advertise to re-generate the packet and re-set the advertising data */
