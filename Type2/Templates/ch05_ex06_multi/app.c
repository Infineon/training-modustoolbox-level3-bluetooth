...
/* Code used for current_slot when no device is currently connected */
#define NONE (255)
...
/* NVRAM locations */
/* Each remote device will have its own VSID to hold its bonding information */
#define VSID_BONDINFO_0				(WICED_NVRAM_VSID_START)
#define VSID_LOCAL_IDENTITY_KEYS	(WICED_NVRAM_VSID_START + BOND_MAX)
#define VSID_MULTI_BOND_INFO		(WICED_NVRAM_VSID_START+ 1 + BOND_MAX)
...
uint8_t		current_slot = 	NONE;					/* Slot in bondinfo that has the information for the currently bonded device */
uint8_t		new_device =	WICED_TRUE;		/* Keep track of whether a device being bonded is new or replacing an existing device's information */
    ...
    switch( event )
    {
		case BTM_ENABLED_EVT:								// Bluetooth Controller and Host Stack Enabled
			if( WICED_BT_SUCCESS == p_event_data->enabled.status )
			{
				...
				/* Read data from nvram into the multi_bond structure in RAM */
				/* If no client has bonded previously, then the read will fail */
				memset( &multi_bond, 0, sizeof(multi_bond) );
				wiced_hal_read_nvram( VSID_MULTI_BOND_INFO, sizeof(multi_bond), (uint8_t*)&multi_bond, &nvram_status);
				if( nvram_status == WICED_SUCCESS )
				{
					WICED_BT_TRACE("\tNumber of bonded devices: %d, next slot: %d\n", multi_bond.num_bonded, multi_bond.next_slot);
				}
				else
				{
				...
				if(multi_bond.num_bonded > 0)
				{
					for (count = 0; count < multi_bond.num_bonded; count++)
					{
						wiced_hal_read_nvram( VSID_BONDINFO_0 + count, sizeof(bondinfo[count]), (uint8_t*)&bondinfo[count], &nvram_status);
						if( nvram_status == WICED_SUCCESS )
						{
							wiced_bt_dev_add_device_to_address_resolution_db ( &(bondinfo[count].link_keys) );
							WICED_BT_TRACE("\tBonding info found in NVRAM. Link key added to address resolution database for:\n" );

							/* Print existing bonding info */
							WICED_BT_TRACE("Remote BDA: ");
							print_array(&(bondinfo[count].link_keys.bd_addr), sizeof(bondinfo[count].link_keys.bd_addr));
							WICED_BT_TRACE("CCCD: ");
							print_array(&(bondinfo[count].cccd), sizeof(bondinfo[count].cccd));
							WICED_BT_TRACE("Link Keys: ");
							print_array(&(bondinfo[count].link_keys), sizeof(bondinfo[count].link_keys));
						}
					}
		...
		case BTM_ENCRYPTION_STATUS_EVT: 						// Encryption Status Event
	        ...
	        if( bond_mode == BONDED)
	    	{
				...
				for(count = 0; count < multi_bond.num_bonded; count++)
				{
					if(memcmp( bondinfo[count].link_keys.bd_addr, p_event_data->encryption_status.bd_addr, sizeof(wiced_bt_device_address_t))==0) /* Matching address found */
					{
						current_slot = count;
			    		/* Set CCCD value from the value that was previously saved in the NVRAM */
			    		app_mysvc_counter_client_char_config[0] = bondinfo[current_slot].cccd[0];
			    		app_mysvc_counter_client_char_config[1] = bondinfo[current_slot].cccd[1];
				    	WICED_BT_TRACE("Restored saved CCCD info for slot %d: %d %d\n",current_slot, bondinfo[current_slot].cccd[0],bondinfo[current_slot].cccd[1]);
	        ...
	        {
	    		app_mysvc_counter_client_char_config[0] = 0;
	    		app_mysvc_counter_client_char_config[1] = 0;
	        }
			break;
		...
		case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT: 			// Save link keys
			/* Assume this device will go in a new slot */
			new_device = WICED_TRUE;
			current_slot = multi_bond.next_slot;
			/* Check to see if we already have data for a device with this BDA in an existing slot */
			for(count = 0; count < multi_bond.num_bonded; count++)
	        {
	        	if(memcmp( bondinfo[count].link_keys.bd_addr, p_event_data->paired_device_link_keys_update.bd_addr, sizeof(wiced_bt_device_address_t))==0) /* Matching address found */
	        	{
	        		new_device = WICED_FALSE;
	        		current_slot = count;
					WICED_BT_TRACE("Device with this address found in %d, will overwrite with new key\n", count);
					break; /* we found what we need so stop searching */
	        	}
	        }

			/* Save link keys in the correct slot */
			memcpy(&(bondinfo[current_slot].link_keys), &(p_event_data->paired_device_link_keys_update), sizeof( wiced_bt_device_link_keys_t));
			memset(&(bondinfo[current_slot].cccd), 0, sizeof(bondinfo[current_slot].cccd));
			wiced_hal_write_nvram ( VSID_BONDINFO_0 + current_slot, sizeof(bondinfo[current_slot]), (uint8_t*)&(bondinfo[current_slot]), &nvram_status );
			if(nvram_status == WICED_SUCCESS)
			{
				WICED_BT_TRACE( "\tBonding info saved to NVRAM to slot %d for BDA %B result: %d \n\r", current_slot, (uint8_t*)&(bondinfo[current_slot].link_keys), nvram_status );
			}
			else
			{
				WICED_BT_TRACE("NVRAM Link Keys Write Error: %d\n", nvram_status);
				status = WICED_BT_ERROR;
			}

			/* Add device to the address resolution database */
			wiced_bt_dev_add_device_to_address_resolution_db ( &(bondinfo[current_slot].link_keys) );
			...
			if(new_device)
			{
				multi_bond.num_bonded++;
				multi_bond.next_slot = (multi_bond.next_slot + 1) % BOND_MAX; /* Increment but wrap around when we reach BOND_MAX */

				wiced_hal_write_nvram ( VSID_MULTI_BOND_INFO, sizeof(multi_bond), (uint8_t*)&(multi_bond), &nvram_status );
				if(nvram_status != WICED_SUCCESS)
				{
					WICED_BT_TRACE("NVRAM Multi Bond Info Write Error: %d\n", nvram_status);
					status = WICED_BT_ERROR;
				}
			}
			break;
		...
		case BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT: 		// Retrieval saved link keys
			/* Search the existing bondinfo to see if this device already has bonding info */
			current_slot = NONE; /* Assume device not found in the list */

			for(count = 0; count < multi_bond.num_bonded; count++)
	        {
	        	if(memcmp( bondinfo[count].link_keys.bd_addr, p_event_data->paired_device_link_keys_request.bd_addr, sizeof(wiced_bt_device_address_t))==0) /* Matching address found */
	        	{
	        		current_slot = count;
					memcpy(&(p_event_data->paired_device_link_keys_request), &(bondinfo[current_slot].link_keys), sizeof(wiced_bt_device_link_keys_t));
					WICED_BT_TRACE("Stored link keys found in slot %d\n", count);
            ...
            if (validLen)
            {
                ...
                switch ( attr_handle )
                {
                	...
                	case HDLD_MYSVC_COUNTER_CLIENT_CHAR_CONFIG:
                		...
                		bondinfo[current_slot].cccd[0] = p_val[0];
                		bondinfo[current_slot].cccd[1] = p_val[1];

                		/* Save CCCD to NVRAM */
                		wiced_result_t nvram_status;
                		// Write bonding info for the current device
                		wiced_hal_write_nvram( VSID_BONDINFO_0 + current_slot, sizeof(bondinfo[current_slot]), (uint8_t*)&(bondinfo[current_slot]), &nvram_status );
                		if(nvram_status == WICED_SUCCESS)
                		{
                    		WICED_BT_TRACE( "\tWrite CCCD value to NVRAM for slot %d\n\r", current_slot );
                		}
                		else
                		{
                			WICED_BT_TRACE( "\tNVRAM Write error: %d\n\r", nvram_status );
...
void rx_cback( void *data )
{
    ...
    switch (readbyte)
    {
    	case 'l': /* Print bonded device info */
    		WICED_BT_TRACE("**********************************\n");
    		WICED_BT_TRACE("Number of bonded devices: %d\n", multi_bond.num_bonded);
    		WICED_BT_TRACE("Next slot to store bonding info: %d\n", multi_bond.next_slot);
			for(int i=0; i<BOND_MAX; i++)
			{
				WICED_BT_TRACE("Remote BDA in slot %d: ", i);
				print_array(&(bondinfo[i].link_keys.bd_addr), sizeof(bondinfo[i].link_keys.bd_addr));
				WICED_BT_TRACE("CCCD value:");
				print_array(&(bondinfo[i].cccd), sizeof(bondinfo[i].cccd));

			}
			WICED_BT_TRACE("**********************************\n");
    		break;

    	case 'e': /* Toggle bonding mode */
			if(connection_id == 0) /* Only allow when device is not connected */
			{
				...
				else /* toggle into bonded mode */
				{
					...
					{
						wiced_hal_pwm_change_values( PWM0, LED_TOGGLE_BONDED, PWM_INIT_BONDING );
					}
					WICED_BT_TRACE( "Bonded Mode\n");
				}
			}
			else
			{
				WICED_BT_TRACE("Bonding mode can only be entered when disconnected\n");
			}
			break;
